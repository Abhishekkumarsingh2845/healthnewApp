...
const MainNavigation = () => {
  const Stack = createStackNavigator<RootStackParamList>();
  const realm = useRealm();
  const {init} = useRefreshData(); // Use the hook
  useEffect(() => {
    return () => {
      realm.close();
    };
  ...
  const {setInitialRoute, setIsGettingInitialRoute} = useRoute();
  ...
  useEffect(() => {
    ...
    messaging().onNotificationOpenedApp( async remoteMessage => {
      ...
      setIsGettingInitialRoute(false)
