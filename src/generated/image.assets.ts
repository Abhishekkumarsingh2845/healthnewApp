export const Images ={
    appLogo:require('../../assets/images/app_logo.png'),
}
export const Icons = {
    ic_active_explore:require("../../assets/images/ic_active_explore.png"),
    ic_explore:require("../../assets/images/ic_explore.png"),
    ic_fav:require("../../assets/images/ic_fav.png"),
    ic_profile:require("../../assets/images/ic_profile.png"),
    ic_active_profile:require("../../assets/images/ic_active_profile.png"),
    ic_bell:require("../../assets/images/ic_bell.png"),
    ic_filter:require("../../assets/images/ic_filter.png"),
    ic_search:require("../../assets/images/ic_search.png"),
    ic_fitness:require("../../assets/images/ic_fitness.png"),
    ic_mental_health:require("../../assets/images/ic_mental_health.png"),
    ic_nutrition:require("../../assets/images/ic_nutrition.png"),
    ic_latest:require("../../assets/images/ic_latest.png"),
    ic_like:require("../../assets/images/ic_like.png"),
    ic_love:require("../../assets/images/ic_love.png"),
    ic_active_love:require("../../assets/images/ic_active_love.png"),
    ic_share:require("../../assets/images/ic_share.png"),
    ic_recent:require("../../assets/images/ic_recent.png"),
    ic_notification:require("../../assets/images/ic_notification.png"),
    ic_privacy:require("../../assets/images/ic_privacy.png"),
    ic_tnc:require("../../assets/images/ic_tnc.png"),
    ic_about:require("../../assets/images/ic_about.png"),
    ic_heart:require("../../assets/images/ic_heart.png"),
    ic_move:require("../../assets/images/ic_move.png"),

    // ac_bt_bg:require("../../assets/images/ac_bt_bg.png"),
}

